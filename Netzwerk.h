...
///structs werden hier nur als hübschere alternative zu std::pair genutzt
struct variable
{
    bauteil *v_teil;
    char v_typ;
...
private:
    ...
    std::vector<variable> variablen;            ///speichert, wo welche variable im gleichungssystem steht ('u' für volt, 'i' für ampere) (spaltenbeschriftung)
    std::vector<std::string> variablen_werte;   ///bei freien Var werden hier Ergebnisse eingetragen
    unsigned int n_variablen;                   ///ein bauteil kann auch mehrere unbekannte haben -> muss extra gezählt werden (matrix breite)
    unsigned int n_pivotelemente;               ///gibt an, wie viele variablen berechnet werden können (wird nach gaussen auggestellt)
    ...
    void finde_maschen(std::list<maschenabschnitt> masche);         ///wird in build matrix aufgerufen um rekursiv maschengleichungen aufzustellen (maschen liste zu füllen)
    void gauss_matrix();                                            ///gausst matrix und rechnet variablen aus
    void spaltentausch(int spalte1, int spalte2);                   ///wird bei berechnung der variablen in gauss_matrix benötigt
    bool ergebnis();                                                ///überprüft ob nullzeilen in matrix ergebnis != 0 haben (wenn nicht -> ergebnis = true)
