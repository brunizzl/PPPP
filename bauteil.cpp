...
char widerstand::var_gesucht()
{
    if (n_known == 3) {
        ...
        if (!(delta_ohm < 0.001) || !(delta_ohm > -0.001)) {
            std::cout << "Warnung: Widerstand " << get_name() << " hat einen falsch eingegeben Wert. f�r Rechnung wird ohm ignoriert." << '\n';
            o_known = 0;
        }
        ...
        if (!v_known) {
            ...
            return 'i';             //volt ist bekannt -> ampere und ohm sind unbekannt -> ampere wird per matrix berechntet, ohm per ohmschen gesetz sp�ter
        }
    }
    else {
        return 'b';                 //alle drei werte gesucht -> volt und ampere werden berechntet, ohm dann sp�ter aus den beiden mit ohmschen gesetz
    }
}
...
//r�ckgabe char ist ob zahl in ergebnis kommt (wert bekannt -> char == '\0') oder in eine variablenspalte,
