...
widerstand *netzwerk::add_widerstand(std::string &name, int knoten1, int knoten2)
{
    ...
    return neu;                             ///rückgabe ist pointer auf neuen widerstand
}
...
quelle *netzwerk::add_quelle(std::string &name, int knoten1, int knoten2)
{
    ...
    knoten[knoten1].push_back(neu);         ///trage neue verbindung in graphen unter bauteile vector von knoten 1 ein
    knoten[knoten2].push_back(neu);         ///trage neue verbindung in graphen unter bauteile vector von knoten 2 ein
    return neu;                             ///rückgabe ist pointer auf neue quelle
}
    ...
    if (!datei) {
        ...
        while (std::getline(datei, zeile)) {
            ...
            if (zeile.find("knoten") != std::string::npos) {
                zeile.erase(0, 7);          ///lösche das wort "knoten " inklusive erstes leerzeichen
                ...
                zeile.erase(0, 11);     ///lösche das wort "widerstand " inklusive erstes leerzeichen
                ...
                zeile.erase(0, 7);      ///lösche das wort "quelle " inklusive erstes leerzeichen
...
void netzwerk::build_matrix()
{
    ...
    for (unsigned int nummer = 0; nummer < bauteile.size(); nummer++) {
        ...
        case 'u':
            variablen.push_back({bauteile[nummer], 'u'}); ///vektor mit zu berechnenden variablen wird um neue variable ergänzt
            n_variablen++;
            break;
        case 'i':
    ...
    for (auto i_knoten : knoten) {                                              ///i_knoten ist std::pair<int, std::vector<bauteil *> > (also ein mapelement)
        ...
        for (auto teil : i_knoten.second) {                                     ///teil ist bauteil* aus vektor aus map "knoten"
            ...
            if (wert.v_typ == '\0') {
                ...
                bool gefunden = false;
                for (unsigned int var = 0; var < variablen.size() && !gefunden; var++) {        ///hätte gerne auto genutzt, brauche aber zu beschreibende spalte von matrix (aka var)
...
{
    bool kreis = false;                 ///ist aktuells letzter knoten bereits vorher enthalten? (ist masche vollständiger kreis?)
    int zaehler = 0;                    //zaehlt iteration von for schleife
    int abbruch = masche.size() - 1;    //sorgt für abbruch von for schleife ein schritt vor ende (letztes element soll nicht mit sich verglichen werden)
    for (auto &it : masche) {
        if (zaehler++ < abbruch) {      //(ich weiß selbst, dass das krebs ist, aber habe gerade keinen plan, wie das in echt gehen würde)
    ...
    if (kreis) {
        ...
        /// baue neue masche für jede verzweigung
...
void netzwerk::gauss_matrix()
{
    ...
    for (unsigned int ref_zeile = 0, ref_spalte = 0; ref_zeile < kurze_seite; ref_zeile++, ref_spalte++) {
        ...
        if (pivot) {
            if (pivot_zeile > ref_zeile) {
                ///zeilen müssen getauscht werden
    ...
    ///ergebnis ausrechnen (vorsicht trickologie) (ist etwas ausfürlicher am ende der datei erklärt):
    ...
    for (int zeile = n_pivotelemente - 1; zeile >= 0; zeile--) {
        ...
        ///(pivotelement selbst müsste auch getielt werden, ist dann aber immer 1, lohnt sich also nicht (siehe trickologie))
        for (unsigned int spalte = n_pivotelemente; spalte < n_variablen + 1; spalte++) {
            ...
            if (spalte < n_variablen) {                         ///für freie Variablen (quasi umstellen auf andere seite des gleichs)
                matrix[zeile][spalte] *= -1;
            }
        }

        ///Freie Variablen und ergebnisspalte für zeilen darüber als anteil der gerade berechneten festen variable
        ///auf die bereits vorhandenen werte für freie variablen addieren
    ...
    if (ergebnis()) {
        if (n_variablen == n_pivotelemente) {
            ...
            for (auto var : variablen) {
                ...
                var.v_teil->var_gesucht();  ///var_gesucht ergänzt auch werte, die sich direkt mit der bauteilinternen info berechnen lassen.
                zeile++;
            }
            std::cout << '\n' << "Berechnete Werte:" << '\n' << '\n';
...
///Erklärung Funktionsweise des rückeinsetzens nach dem gaussen:
///(das habe ich alles nicht neu geschrieben, der matrixcode kommt aus einem alten matrixrechner von mir)
///Die Grundidee beim zurückeinsetzen ist, von unten nach oben die variablen auszurechnen und die werte für jede berechnete
...
///(zeile 1 ist hier vorhanden, jedoch nicht aufgeführt. die Matrix hat also 4 variablen bei 3 gleichungen.)
...
///(sonst hätte es bereits eine zeile gegeben, in der x4 alleine steht.)
///Da x3 in x2 vier mal vorkommt, mehmen wir die Werte von x3 mal vier und speichern sie im array in der zeile von x2
///addiert mit den werten für x4 und die konstante die dort vorher schon standen.
///Da der konstante wert auf der anderen seite des gleichzeichens steht,
///müssen wir hier den konstanten wert von x3 (die 5) noch *(-1) nehmen, um ihn auch auf die andere Seite zu bringen.
///unsere eingesetzte Gleichung sieht also so aus: x2 + 6*x4 = -10
///als array brauchen wir also nur noch die speicherplätze von x4 und der konstanten Zahl um x2 darzustellen.
///die anderen arrayeinträge müssen wir nicht mehr verändern:
